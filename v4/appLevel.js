...
const tileType_wool    = 9;
const tileType_stick   = 10;

const tileType_leaf    = 11;
const tileType_log     = 12;

const tileType_dummy   = 13;

const tileType_vine    = 14;
...
function generateTrees() 
{
    ...
    {
	if(Math.floor(rand(3.99,0)) == 0 && abs(checkpointPos.x-x) > 5)
	{	    
	    console.log("tree");
	    let highestSolid = 0;
	    for(let y=levelSize.y; y--;)
	    {
	        let cury = levelSize.y - y;
		if(getTileCollisionData(vec2(x,cury)) != 0)
		{
		    highestSolid = cury;
		}
	    }
	    if(highestSolid != 0)
	    {
		trees[trees.length+1] = vec2(x,highestSolid);
	    }
	}
    }
    ...
    trees.forEach(function(pos)
    {
        for(let i=3; i--;)
        {
	    if(getTileCollisionData(vec2(pos.x,pos.y+i+1))==tileType_empty||getTileCollisionData(vec2(pos.x,pos.y+i+1))==tileType_leaf){
                setTileCollisionData(vec2(pos.x,pos.y+i+1), tileType_log);
	    }
	    
	    if(getTileCollisionData(vec2(pos.x+1-i,pos.y+5))==tileType_empty){
                setTileCollisionData(vec2(pos.x+1-i,pos.y+5),tileType_leaf);
	    }
        }
        for(let i=2; i--;)
        {
	    if(getTileCollisionData(vec2(pos.x+i+1,pos.y+3))==tileType_empty){
    	        setTileCollisionData(vec2(pos.x+i+1,pos.y+3),tileType_leaf);
	    }
	    if(getTileCollisionData(vec2(pos.x-i-1,pos.y+3))==tileType_empty){
    	        setTileCollisionData(vec2(pos.x-i-1,pos.y+3),tileType_leaf);
	    }
        ...
        for(let i=5; i--;)
        {
	    if(getTileCollisionData(vec2(pos.x+2-i,pos.y+4))==tileType_empty){
                setTileCollisionData(vec2(pos.x+2-i,pos.y+4),tileType_leaf);
	    }  
        }
    });
}
...
function generateLevel()
{
    ...
    //choose whether or not to generate trees
    let trees = Math.floor(Math.random()*1.99);
    if (trees == 1) generateTrees();
...
function makeTileLayers(level_)
{
    ...
    for(let y=levelSize.y;y--;)
    {
        ...
        if (tileType)
        {
            ...
            {
                tileIndex = groundTileStart+1 + Math.floor(rand(2.99,0));
	    ...
	    else if (tileType == tileType_wool)
	    {
		tileIndex = groundTileStart+13;
                direction = mirror = 0;
	    }
	    else if (tileType == tileType_stick)
	    {
		tileIndex = groundTileStart+14;
                direction = mirror = 0;
	    }
	    else if (tileType == tileType_leaf)
	    {
		tileIndex = groundTileStart+17;
                direction = mirror = 0;
	    }
	    else if (tileType == tileType_log)
	    {
		tileIndex = groundTileStart+22;
                direction = mirror = 0;
	    }
	    else if (tileType == tileType_dummy)
	    {
		tileIndex = groundTileStart+18;
                direction = mirror = 0;
	    }
	    else if (tileType == tileType_vine)
	    {
		tileIndex = groundTileStart+23;
                direction = mirror = 0;
	    }
